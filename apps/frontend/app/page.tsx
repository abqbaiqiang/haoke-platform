...
export default function Home() {
  ...
  function openCRM(entry?: CRMEntry) { setCRMEntry(entry); setView("crm"); }
  ...
  return <div className="workspace dash-root"><aside><div className="brand"><div className="brand-mark small">松</div><div><strong>松茂经营管理平台</strong><small>数据驱动经营</small></div></div><p className="nav-label">工作空间</p><nav aria-label="主导航"><button aria-current={view === "bi" ? "page" : undefined} onClick={() => setView("bi")}>销售工作台</button><button aria-current={view === "home" ? "page" : undefined} onClick={() => setView("home")}>平台首页</button><button aria-current={view === "account" ? "page" : undefined} onClick={() => setView("account")}>我的账号</button>{user.role_code === "owner" && <button aria-current={view === "staff" ? "page" : undefined} onClick={() => setView("staff")}>人员管理</button>}{["owner", "admin"].includes(user.role_code) && <button aria-current={view === "system" ? "page" : undefined} onClick={() => setView("system")}>系统状态</button>}{["owner", "admin", "finance"].includes(user.role_code) && <button aria-current={view === "data" ? "page" : undefined} onClick={() => setView("data")}>数据中心</button>}<button aria-current={view === "crm" ? "page" : undefined} onClick={() => openCRM()}>客户与待办</button></nav><div className="sidebar-bottom">M3 销售工作台 <span className="dot" /></div></aside>
    <div className="content"><header><span>{roles[user.role_code]}工作空间</span><div><span>{user.display_name}</span><button onClick={signOut} disabled={busy}>退出登录</button></div></header><main className="dashboard">{error && <p role="alert" className="error">{error}</p>}
      ...
      {view === "bi" && <BI role={user.role_code} userId={user.id} openCRM={openCRM} />}
