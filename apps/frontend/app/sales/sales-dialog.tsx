    ...
    {state.kind === "complete" ? <><p>待办「{state.task?.title}」即将完成。建议把和客户的实际沟通记录下来，下次跟进有依据。</p>
      ...
      {["follow", "task"].includes(state.kind) && <>{state.customer || state.task ? <p className="sales-context">{detail.data?.customer.customer_name || state.customer?.customer_name || state.task?.customer_name}{state.task && <small>当前待办：{state.task.title}</small>}</p> : <>
        <div className="sales-picker"><label>查找客户（输入关键字从下拉中选择）<div className="sales-combobox"><input value={q} maxLength={100} placeholder="例如输入“冠”，从下拉里选客户" onChange={e => { setQ(e.target.value); setCbOpen(true); clearTimeout(timer.current); timer.current = window.setTimeout(() => setSearch(e.target.value.trim()), 300); }} onFocus={() => setCbOpen(true)} onBlur={() => window.setTimeout(() => setCbOpen(false), 150)} />
          {cbOpen && <ul className="sales-combobox-list">{state.kind === "task" && <li><button type="button" aria-pressed={!cid} onMouseDown={e => { e.preventDefault(); setCid(""); setQ(""); setSearch(""); }}>不关联客户（个人待办）</button></li>}{choices.data?.rows.slice(0, 8).map(c => <li key={c.id}><button type="button" aria-pressed={cid === c.id} onMouseDown={e => { e.preventDefault(); setCid(c.id); setQ(c.customer_name); setSearch(c.customer_name); setCbOpen(false); }}>{c.customer_name}<small>{c.customer_code || ""}{c.contact_name ? ` · ${c.contact_name}` : ""}</small></button></li>)}{choices.data?.rows.length === 0 && <li className="sales-combobox-empty">{search ? "没有匹配的客户，换个关键字试试。" : "输入关键字联想客户。"}</li>}{(choices.data?.total || 0) > 8 && <li className="sales-combobox-empty">共 {choices.data?.total} 个匹配，输入更多关键字可缩小范围。</li>}</ul>}
        </div></label></div>
