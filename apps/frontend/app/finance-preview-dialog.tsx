...
/** 财务预览空白科目显示“未填报（空白）”。 */
const blankMoney = (value: string | null) => money(value, { empty: "未填报（空白）" });
  ...
  return <dialog ref={dialog} className="orders-dialog" aria-labelledby="finance-preview-title" onClose={onClose}>
    ...
    <div className="orders-dialog-body">
      ...
      {preview && <><h3>{preview.filename}</h3><p>{preview.entity_name} · {preview.period.slice(0, 7)} · 单位：元</p>
        ...
        <div className="table-scroll"><table><thead><tr><th>原始行</th><th>科目</th><th>{profit ? "本月金额（元）" : "期末余额（元）"}</th><th>{profit ? "本年累计（元）" : "年初余额（元）"}</th></tr></thead><tbody>{preview.records.map(row => <tr key={row.metric_code}><td>{row.row}</td><td>{row.metric_name}</td><td>{blankMoney(profit ? row.period_value : row.end_value)}</td><td>{blankMoney(profit ? row.ytd_value : row.begin_value)}</td></tr>)}</tbody></table></div>
