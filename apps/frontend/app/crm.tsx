...
export default function CRM({role, userId, entry, embedded=false}: {role: Role; userId: string; entry?: CRMEntry; embedded?:boolean}) {
  ...
  return <section className="crm"><p className="eyebrow">工作空间 / 轻量 CRM</p><h1>{selected ? "客户 360" : "客户与待办"}</h1><p className="muted">记录客户关系和下一步安排。正式交易以精斗云为准。</p>
    ...
    {!selected&&((["owner","admin"].includes(role)&&tab==="settings")||(processWrite&&config?.sales_create_tags&&tab==="customers"))&&<section className="card data-section"><Editor title="维护公共标签" fields={[{key:"tag_name",label:"标签名称",required:true,maxLength:100},{key:"tag_group",label:"标签组",maxLength:32}]} submit="新增标签" save={d=>save("/tags","POST",d)}/>{["owner","admin"].includes(role)&&<div className="tag-manage-list">{tags.map(t=><span key={t.id} className={`tag-manage-chip ${t.is_active?"":"off"}`}>
            <strong>{t.tag_name}</strong>{t.tag_group&&<small>{t.tag_group}</small>}
            <button disabled={busy} title={t.is_active?"停用（软隐藏，可恢复）":"启用"} onClick={()=>act(()=>api(`/tags/${t.id}`,"PUT",{tag_name:t.tag_name,tag_group:t.tag_group,is_active:!t.is_active}))}>{t.is_active?"停用":"启用"}</button>
            <button className="danger" disabled={busy} title="彻底删除（同时从所有客户移除，不可恢复）" onClick={()=>{if(window.confirm(`确定删除标签「${t.tag_name}」吗？
将从所有客户上移除该标签，删除不可恢复（留痕保留）。
如只是暂时不用，建议用“停用”。`))act(async()=>{await api(`/tags/${t.id}`,"DELETE");},"标签已删除并留痕")}}>删除</button>
          </span>)}</div>}</section>}
  </section>;
}
