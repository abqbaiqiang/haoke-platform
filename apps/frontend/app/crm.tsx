...
const stageFlow: [string, string][] = [["initial","需求沟通"],["demand","方案沟通"],["quoted","报价"],["negotiating","谈判"],["won","成交"]];
const lifecycle: [string, string][] = [["prospect","潜在"],["active","活跃"],["dormant","沉睡"],["lost","流失"]];
...
function dayDiff(value: string) { const bj = (d: Date) => new Date(d.getTime()+8*3600000).toISOString().slice(0,10); return Math.round((Date.parse(bj(new Date(value)))-Date.parse(bj(new Date())))/86400000); }
    ...
    {selected&&detail&&customer&&<><div className="cd">{!embedded&&<div className="cd-topline"><button onClick={()=>{setSelected(null);rememberCustomer(null);setDetail(null);setEdit(null);}}>← 返回客户列表</button></div>}
      <header className="cd-card cd-summary" aria-label="客户摘要">
        ...
        <dl className="cd-meta">
          ...
          <div><dt>创建时间</dt><dd>{fmt(customer.created_at)}</dd></div>
        </dl>
        <div>
      ...
      {detailTab==="overview"&&<><div className="cd-kpis">
        ...
        <button className="cd-card cd-kpi" onClick={()=>setDetailTab("followups")}><span>最近跟进</span><strong>{lastFollow?relLabel(dayDiff(lastFollow.occurred_at)):"—"}</strong><small>{lastFollow?fmt(lastFollow.occurred_at):"暂无跟进记录"}</small></button>
        <button className="cd-card cd-kpi" onClick={()=>setDetailTab("tasks")}><span>下一步计划</span><strong className={nextTask&&dayDiff(nextTask.due_at)<0?"danger":""}>{nextTask?relLabel(dayDiff(nextTask.due_at)):"—"}</strong><small>{nextTask?fmt(nextTask.due_at):"暂无待办"}</small></button>
      </div>
        ...
        <div className="cd-main">
          <section className="cd-card" aria-label="下一步行动">
            ...
            {nextTask?<div className={"cd-next"+(dayDiff(nextTask.due_at)<0?" overdue":"")}>
              <div className="when">{fmt(nextTask.due_at)}<br/><span className="due">{dayDiff(nextTask.due_at)<0?`已逾期 ${-dayDiff(nextTask.due_at)} 天`:relLabel(dayDiff(nextTask.due_at))}</span></div>
