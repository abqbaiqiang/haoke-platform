...
export default function DataCenter({ role }: { role: string }) {
  ...
  useEffect(() => { act(async () => { await refreshSources(); await refreshBatches(0); if (role === "admin") setUsers(await api("/mapping-users")); }); }, []); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return <><p className="eyebrow">工作空间 / 数据中心</p><h1>数据中心</h1><p className="muted">先预检，再确认导入。每个批次保留原件和核对记录。</p>
    ...
    <div className="data-tabs"><button aria-pressed={tab === "imports"} onClick={() => setTab("imports")}>文件导入与历史</button><button aria-pressed={tab === "reconcile"} onClick={() => setTab("reconcile")}>{role === "admin" ? "财务期间确认" : "月度核对"}</button>{role === "admin" && <button aria-pressed={tab === "settings"} onClick={() => setTab("settings")}>数据源与人员映射</button>}</div>
    ...
    {tab === "settings" && role === "admin" && <><section className="card data-section"><h2>创建数据源</h2><form onSubmit={e => { e.preventDefault(); const f = new FormData(e.currentTarget); act(async () => { const created = await api("/sources", json({ source_code: f.get("code"), source_name: f.get("name"), entity_name: f.get("entity") })); await refreshSources(); setSource(created.id); setMessage("数据源已创建，可以上传文件。"); }); }}>
    ...
    {tab === "imports" && <>{canWrite && <section className="card data-section"><h2>上传并预检</h2><form onSubmit={upload}><label>文件类型<select aria-label="文件类型" value={kind} onChange={e => setKind(e.target.value)}>{Object.entries(kinds).filter(([k]) => role === "admin" || ["profit", "balance_sheet"].includes(k)).map(([k, label]) => <option value={k} key={k}>{label}</option>)}</select></label>
