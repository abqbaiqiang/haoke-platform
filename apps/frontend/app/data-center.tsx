...
import { importKindLabels as kinds, importStatusLabels as status } from "./lib/labels";
...
export default function DataCenter({ role }: { role: string }) {
  ...
  return <><p className="eyebrow">工作空间 / 数据中心</p><h1>数据中心</h1><p className="muted">先预检，再确认导入。每个批次保留原件和核对记录。</p>
    ...
    {tab === "settings" && ["admin", "owner"].includes(role) && <><section className="card data-section"><h2>创建数据源</h2><form onSubmit={e => { e.preventDefault(); const f = new FormData(e.currentTarget); act(async () => { const created = await api("/sources", json({ source_code: f.get("code"), source_name: f.get("name"), entity_name: f.get("entity") })); await refreshSources(); setSource(created.id); setMessage("数据源已创建，可以上传文件。"); }); }}>
      ...
      {source && <section className="card data-section"><h2>清空导入数据（重来）</h2><p>删除该数据源已导入的销售订单、明细和客户／商品档案；有跟进、认养等 CRM 记录的客户会停用保留而不是删除。导入批次、原始文件和操作审计都会保留。清空后请按“客户／商品 → 销售”的顺序重新导入；同一份文件重复导入不会产生重复数据。</p>
        <label>输入数据源名称“{sources.find(s => s.id === source)?.source_name}”以确认<input value={purgeText} onChange={e => setPurgeText(e.target.value)} maxLength={100} placeholder="输入数据源名称确认" /></label>
        <button className="primary" disabled={busy || purgeText.trim() !== sources.find(s => s.id === source)?.source_name} onClick={() => act(async () => { const r = await api(`/sources/${source}/data?confirm=${encodeURIComponent(purgeText.trim())}`, { method: "DELETE" }) as { orders: number; customers_removed: number; customers_deactivated: number; products_removed: number }; setPurgeText(""); setMessage(`已清空：销售订单 ${r.orders} 笔、客户删除 ${r.customers_removed}／停用 ${r.customers_deactivated}、商品删除 ${r.products_removed}。可重新导入。`); })}>清空该数据源的导入数据</button></section>}</>}
    {tab === "imports" && <>{canWrite && <section className="card data-section"><h2>上传并预检</h2><form onSubmit={upload}><label>文件类型<select aria-label="文件类型" value={kind} onChange={e => setKind(e.target.value)}>{Object.entries(kinds).filter(([k]) => ["admin", "owner"].includes(role) || ["profit", "balance_sheet"].includes(k)).map(([k, label]) => <option value={k} key={k}>{label}</option>)}</select></label>
      ...
      <section className="card data-section"><h2>导入历史</h2><div className="table-scroll"><table><thead><tr><th>文件</th><th>类型</th><th>状态</th><th>时间</th><th>操作</th></tr></thead><tbody>{batches.map(b => <tr key={b.id}><td>{b.original_filename}</td><td>{kinds[b.business_type]}</td><td>{status[b.status]}</td><td>{dateTime(b.started_at)}</td><td><button onClick={() => { setSelected(b); setRaw([]); setAck(false); setReplace(false); }}>查看批次</button>{b.status === "pending" && canWrite && !["profit", "balance_sheet"].includes(b.business_type) && <button className="confirm-inline" disabled={busy} onClick={() => act(async () => { const done = await api(`/imports/${b.id}/confirm`, json({ acknowledge_warnings: true, replace_version: false })); await refreshBatches(offset); const result = done.preview?.result; setMessage(`导入完成：新增 ${result?.inserted ?? 0} · 更新 ${result?.updated ?? 0} · 未变化 ${result?.unchanged ?? 0}。`); })}>确认导入</button>}{["pending", "failed"].includes(b.status) && ["admin", "owner"].includes(role) && <button disabled={busy} onClick={() => { if (!window.confirm(`删除批次“${b.original_filename}”的预检记录与原始文件？已导入的数据不受影响。`)) return; act(async () => { await api(`/imports/${b.id}`, { method: "DELETE" }); await refreshBatches(offset); setMessage("批次已删除，原始文件一并移除；已导入的数据不受影响。"); }); }}>删除</button>}</td></tr>)}</tbody></table></div>{!batches.length && <p>还没有导入记录。</p>}<button disabled={busy || offset === 0} onClick={() => act(async () => { const n = Math.max(0, offset-20); setOffset(n); await refreshBatches(n); })}>上一页</button><button disabled={busy || batches.length < 20} onClick={() => act(async () => { setOffset(offset+20); await refreshBatches(offset+20); })}>下一页</button></section>
    </>}
