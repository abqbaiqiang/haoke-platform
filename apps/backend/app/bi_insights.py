"""BI 指标层：销售分析/工作台/客户关注/客户画像与经营总览的指标实现（docs/29 C4 拆分）。

口径统一引用 app.bi_caliber；数据装载与权限引用 app.bi_access；
指标码与公式仅调用 app.bi_calculations，不改写任何核心数字。
"""
...
from app.bi_access import TZ, load_orders, month, person, require, scope, settings, source
...
from app.constants import FULL_ACCESS_ROLES, ROLE_ADMIN, ROLE_FINANCE, ROLE_MANAGER, ROLE_OWNER, ROLE_SALES, \
    TARGET_MONTHLY, TARGET_QUARTERLY
