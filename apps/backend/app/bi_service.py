...
docs/29 C4 拆分进行中：装载层（权限/配置/目标/数据源/订单加载）已拆至 app.bi_access；
本文件 re-export 既有公开名，bi_api/sales_workspace/测试的调用面不变。
后续步骤：口径 -> app.bi_caliber，指标 -> app.bi_insights；DTO 即 app.bi_schemas。
"""
...
from app.bi_access import TZ, audit, get_target, latest_fact, load_orders, month, people, person, require,     save_review, save_settings, save_target, scope, settings, source, sources
...
from app.constants import FULL_ACCESS_ROLES, ROLE_ADMIN, ROLE_FINANCE, ROLE_MANAGER, ROLE_OWNER, ROLE_SALES,     TARGET_MONTHLY, TARGET_QUARTERLY
...
__all__ = [
    'TZ', 'audit', 'get_target', 'latest_fact', 'load_orders', 'month', 'people', 'person', 'require',
    'save_review', 'save_settings', 'save_target', 'scope', 'settings', 'source', 'sources',
    'order_value', 'included', 'normal_sale', 'analysis', 'orders', 'workbench', 'attention',
    'RFM_SEGMENTS', 'CONVERT_BUCKETS', 'customer_analytics', 'customer_profile', 'overview',
]
