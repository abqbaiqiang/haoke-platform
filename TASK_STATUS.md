...
- [x] Regression per phase: ruff clean, pytest 279 passed (6 new tests in test_sales_refactor.py + 2 rewritten; the single full-run failure each time was the documented pre-existing flake test_opportunity_products_roundtrip_and_recent_list, passing on isolated reruns), tsc/next build/bundle green, sales E2E desktop+mobile 2/2 per phase; local DB upgraded 0011→0013 and backend restarted after each backend change
