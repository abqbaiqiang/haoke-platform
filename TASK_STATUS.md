...
- [x] Backend: GET/PUT /api/crm/customers/{cid}/location reusing existing customer view/edit permission (finance GET 200 / PUT 403; nonexistent/out-of-scope unified 404); server writes status=located + updated_at/by (last write wins, no client-supplied modifier); audit event customer_location_update with before/after coordinates; invalid lat/lng/source 422. Map proxy GET /api/map/geocode (server holds TENCENT_MAP_SERVER_KEY, never sent to browser; clear Chinese errors for unconfigured 503 / no-result 404 / timeout 502, never a fake city-center coordinate) + GET /api/map/config (web JS key only); Settings tencent_map_web_key/tencent_map_server_key (SecretStr, default empty = gracefully disabled); .env.example documents key application + whitelists
- [x] Frontend: customer-360 overview adds 客户位置 card between 客户信息 and 客户备注 (only .cd-card/.cd-info/--cd-* tokens reused, no new visuals); viewer modal (read-only, Modal gained wide 920px variant) + editor modal (address search defaults to company_address, results never auto-saved; map click=map_click, marker drag=map_drag; cancel writes nothing); Tencent GL JS SDK injected on demand (ECharts-style dynamic script load); without keys card/detail stay fully usable ("地图暂时无法加载" degradation, no white screen)
- [x] Tests: tests/test_customer_location.py (5 cases: unset default, save+modifier+audit, invalid coords/source 422, nonexistent/out-of-scope 404 + finance read-only, map config secret hygiene + geocode mock paths); e2e/crm-location.spec.ts (sales workspace → unset card → degraded editor → same PUT API → located card + coords + no overflow); full pytest 290 passed (sole failure = documented pre-existing flake test_opportunity_products_roundtrip, passes isolated), ruff/tsc/next build green; local dev DB upgraded to 0014
- [ ] Pending owner: apply Tencent LBS keys (Web JS key with domain whitelist + WebService key with server IP whitelist) into .env, then manually accept real map interactions (doc Cases 3-5 search/click/drag) — not automatable without keys
- [ ] Note: PUT endpoint ready for the future miniapp (location_source=miniapp reserved, same permission model); coordinate-field merge decision with docs/miniapp dev package deferred to Phase 7 per docs/miniapp/DEV_LOG

## 2026-09-19 WeChat mini-program CRM started (docs/miniapp dev package, Phase 0+1 done)

- [x] Phase 0 baseline (commit 9860760, no business code touched): dev package 00-06 docs + 6 UI reference images archived into `docs/miniapp/`; baseline verified against code — reuse Customer/Contact/Followup/Task/Opportunity/FollowupAttachment models, `save_followup()` task linkage, `customer_scope`, cookie auth + Origin middleware, /api/map tencent proxy (web customer-location feature landed uncommitted by a parallel workstream); worker currently heartbeat-only (geocode/ASR polling jobs to be added); output `docs/miniapp/DEV_LOG.md` with 5 required sections + 8 risks (top: coordinate-field merge decision needed before Phase 7 since uncommitted 0014_customer_location already added latitude/longitude; /api/mobile Origin exemption design; Taro4+React POC)
- [x] Phase 1 scaffold (apps/miniapp): Taro 4.2.1 + React 18.3.1 (Taro-compatible, React 19 deferred) + TS5.6 + SCSS; tokens.scss on `page` selector aligned with PC DESIGN.md; 15 shared components (Avatar/Tag/SectionCard/TaskRow/CustomerRow/CustomerHeader/VisitBrief/AISummaryCard/FollowupTimelineCard/OrderTimelineCard/TimelineItem/NearbyCustomerRow/BrandHeader/EmptyState/LoadingState/ErrorState); 7 pages (login/workbench/customers/customer-detail/followup-edit/nearby/mine) with native 4-tab tabBar + PIL-generated line icons; mock centralized in services/mock.ts (types = future Mobile DTO contract); no BI, no add-customer entry, empty-state copy directs to existing systems; build:weapp + typecheck green (1 known css-order warning); VoiceRecorder/MediaPicker/UploadStrip deferred to Phase 4/5 per 06 sequencing
- [x] Regression: scripts/run_tests.py full gate passed (ruff + pytest 290 + frontend tsc/next build + bundle secret scan); backend untouched by this phase
- [ ] Phase 2 next: mobile auth (Bearer session, /api/mobile Origin exemption in request_boundary, PC cookie regression tests)
- [ ] Phases 3-9 queued per docs/miniapp/06 (read-only → followup write → media → ASR/AI → map → visual polish → full regression); ASR/AI vendor selection needed before Phase 6; ui/05 "route banner" conflicts with PRD V1 scope (docs-first: degrade to plain hint, confirm with owner before Phase 7); coordinate-field merge with web location feature confirmed deferred to Phase 7 by both sides
...
- [x] Phase 2 Workbench: all-colleague project panel (GET /api/sales/opportunities/board, owner names + product submissions + stage/probability%, others' projects read-only — S1-write-S2 403/404 tested); 4 KPI cards via GET /api/sales/workbench-summary (target/completion, today+overdue tasks, open projects all staff, key customers = RFM 重要层 via bi.rfm_key_customers, no fake zeros); 今日作战区 (customer/item/stage[open opp top stage via TaskRow.opp_stage]/due/去跟进+完成+调整); attention click-through (沉睡→status=dormant, 疑似流失→at_risk, overdue→tasks); quick-follow image PASTE (migration 0012 crm_followup_attachment, base64 upload API, png/jpeg/webp whitelist, followup_image_max_mb configurable limit, sha256 dedup storage, owner-only; paste UI with thumbnails; attachment lists in sales recent + crm followups); recent-followups dynamic position (today tasks ≥6 moves down, CSS order)
- [x] Phase 3 Customer caliber (migration 0013): customer_status column dropped (test data, owner-authorized; auto quotation→quoted rewrite and sales whitelist swapped for company_address); customers API status param + tab cascade (dormant/deal/quoting[intent by project stage bidding/negotiation]/intent/new per docs/32 §3.2, at_risk + key extras; manually-lost excluded from tabs, gray in 全部); frontend tab strip with counts, key/at_risk banner, CRM detail 阶段 row → 公司地址, customer_status removed from labels/types/editors; tests rewritten + cascade专项 (6-bucket + at_risk overlap + key per-capita + 422); fixed residual local-import UnboundLocalError 500
- [x] Phase 4 Tasks kanban: 4 stat cards + view tabs 全部/今天/逾期/未来7天/已完成 (default all = board); three columns 今天必须完成/已逾期/未来计划 with per-card complete checkbox/记录跟进/调整/设为今天 (PATCH due 18:00); empty-overdue column collapses and remaining two expand; single views keep paginated table; e2e selectors adapted (board cards, 全部未来→未来7天)
- [x] Phase 5 Performance: funnel = 4 distinct-customer stages (新增客户=建档口径 with prev-month delta, 有效沟通=effective followup customers, 报价=quotation_sent, 成交=verified orders); quality quality dict (新增/活跃/沉睡/复购) replacing 客户结构 panel as 客户质量 tiles (old/new sales share bar kept); TOP5 self-only (existing self scope)
- [x] Regression per phase: ruff clean, pytest 279 passed (6 new tests in test_sales_refactor.py + 2 rewritten; the single full-run failure each time was the documented pre-existing flake test_opportunity_products_roundtrip_and_recent_list, passing on isolated reruns), tsc/next build/bundle green, sales E2E desktop+mobile 2/2 per phase; final full E2E suite 38 passed/10 skipped/0 failed (bi.spec workbench assert synced .wb-overview→.wb-kpis); local DB upgraded 0011→0013 and backend restarted after each backend change
- [ ] Pending owner: visual/business acceptance of the five phases (screenshots); docs/31 admin-console acceptance still separate
- [ ] Phase 6 (final full-site UI consistency audit) deferred until both consoles are final per AGENTS 12.5
- [ ] Note: festival-cycle reminders (去年春节采购类) remain a separate unstarted backend task; quick-follow intentionally has no file-picker upload
